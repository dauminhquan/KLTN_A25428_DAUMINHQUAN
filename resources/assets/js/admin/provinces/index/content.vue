<template>
    <div class="content-wrapper" id="content-wrapper">
        <data-table title="Quản lý thành phố"
                    :columns="columns"
                    :data="data"
                    :targets="[]"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-info">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h6 class="modal-title"><i class="icon-info3"></i> Thông tin thành phố</h6>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content text-center">
                    <div class="modal-header">
                        <h5 class="modal-title">Thêm tỉnh/thành  bằng Excel</h5>
                    </div>
    ...
    export default {
        ...
        data(){
            return {
                ...
                menu: [
                    ...
                    {
                        action :'delete',
                        html:'<a href="javascript:void(0);"><i class="icon-trash"></i> Xóa thành phố</a>'
                    }
                ],
                primaryKey: 'id',
        ...
        methods: {
            ...
            getData(perPage=500,page=1){
                var vm = this
                axios.get(vm.config.API_ADMIN_PROVINCES_RESOURCE+'?size='+perPage+'&page='+page).then(data => {
            ...
            createItem(){
                let vm = this
                vm.creating = true
                axios.post(vm.config.API_ADMIN_PROVINCES_RESOURCE,{
                    name:vm.nameCreate,
                    code: vm.codeCreate
                }).then(data => {
            ...
            updateItem(){
                ...
                {
                    vm.updating = true
                    axios.put(vm.config.API_ADMIN_PROVINCES_RESOURCE+'/'+vm.codeUpdating,vm.info).then(data => {
                        vm.config.notifySuccess('Update thông tin thành phố thành công')
                        vm.updating = false
            ...
            deleteItem(){
                ...
                {
                    let indexOf = -1
                    axios.delete(vm.config.API_ADMIN_PROVINCES_RESOURCE+'/'+vm.primaryKeyDelete).then(data => {
            ...
            deleteListItem() {
                let vm = this
                vm.deleting = true
                axios.delete(vm.config.API_ADMIN_PROVINCES_DELETE_LIST,{
                    params:{
