<template>
    <div class="content-wrapper" id="content-wrapper">
        <data-table title="Quản vị trí công việc"
                    :columns="columns"
                    :data="data"
                    :targets="[]"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-info">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h6 class="modal-title"><i class="icon-info3"></i> Thông tin vị trí</h6>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            getData(perPage=500,page=1){
                var vm = this
                axios.get(vm.config.API_ADMIN_POSITIONS_RESOURCE+'?size='+perPage+'&page='+page).then(data => {
            ...
            createItem(){
                let vm = this
                vm.creating = true
                axios.post(vm.config.API_ADMIN_POSITIONS_RESOURCE,{
                    name:vm.nameCreate
                }).then(data => {
            ...
            updateItem(){
                let vm = this
                vm.updating = true
              axios.put(vm.config.API_ADMIN_POSITIONS_RESOURCE+'/'+vm.info.id,{
                  name:vm.info.name
              }).then(data => {
                  vm.config.notifySuccess('Update thông tin vị trí thành công')
                  vm.updating = false
            ...
            deleteItem(){
                ...
                {
                    let indexOf = -1
                    axios.delete(vm.config.API_ADMIN_POSITIONS_RESOURCE+'/'+vm.primaryKeyDelete).then(data => {
            ...
            deleteListItem() {
                let vm = this
                vm.deleting = true
                axios.delete(vm.config.API_ADMIN_POSITIONS_DELETE_LIST,{
                    params:{
