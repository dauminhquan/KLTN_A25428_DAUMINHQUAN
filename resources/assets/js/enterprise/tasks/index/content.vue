<template>
    <div class="content-wrapper" id="content-wrapper">
        <data-table title="Quản lý việc làm"
                    :columns="columns"
                    :data="data"
                    :targets="[]"
    ...
    export default {
                ...
                buttonConfig: [
                    {
                        ...
                        action: function(e, dt, node, config) {
                            window.open(window._config.WEB_ENTERPRISE_TASKS+'/create')
                        }
                    },
                ],
                deleting: false,
        ...
        methods: {
            ...
            getData(perPage=500,page=1){
                var vm = this
                axios.get(vm.config.API_ENTERPRISE_TASKS_RESOURCE+'?size='+perPage+'&page='+page).then(data => {
            ...
            deleteItem(){
                ...
                {
                    let indexOf = -1
                    axios.delete(vm.config.API_ENTERPRISE_TASKS_RESOURCE+'/'+vm.primaryKeyDelete).then(data => {
            ...
            deleteListItem() {
                let vm = this
                vm.deleting = true
                axios.delete(vm.config.API_ENTERPRISE_TASKS_DELETE_LIST,{
                    params:{
            ...
            showItem(id) {
                let vm = this
                window.open(vm.config.WEB_ENTERPRISE_TASKS+'/'+id+'/edit','_blank')
            },
